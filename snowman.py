...
while run:
    ...
    root.title('Dont melt the Snowman!')
    ...
    for i in range(0,len(selected_word)):
        x += 60
        exec('d{}=Label(root,text="_",bg="#6FA8DC",font=("arial",40))'.format(i))
    ...
    for snowman in h123:
        exec('{}=PhotoImage(file="{}.png")'.format(snowman,snowman))
    ...
    for q1 in button:
        exec('{}=Button(root,bd=0,command=lambda:check("{}","{}"),bg="#6FA8DC",activebackground="#6FA8DC",font=10,image={})'.format(q1[0],q1[1],q1[0],q1[1]))
        exec('{}.place(x={},y={})'.format(q1[0],q1[2],q1[3]))
        
    #snowman pictures placement
    han = [['c1','h1'],['c2','h2'],['c3','h3'],['c4','h4'],['c5','h5'],['c6','h6'],['c7','h7']]
    for p1 in han:
        exec('{}=Label(root,bg="#6FA8DC",image={})'.format(p1[0],p1[1]))

    ## THis is where my problem with placement is occuring. I am planning on defining c1 and or adding manual coordinates above.    
    ...
    def close():
        global run
        answer = messagebox.askyesno('EXIT','Do you want to quit?')
        if answer == True:
            run = False
            root.destroy()
            
    e1 = PhotoImage(file = 'exit.png')
    ex = Button(root,bd = 0,command = close,bg="#6FA8DC",activebackground = "#6FA8DC",font = 10,image = e1)
    ex.place(x=770,y=10)
    s2 = 'SCORE:'+str(score)
    s1 = Label(root,text = s2,bg = "#6FA8DC",font = ("arial",25))
    s1.place(x = 10,y = 10)
